...
//    }
    ...
    public HashMap<Integer, Integer> getCart(int accountId) {
        HashMap<Integer, Integer> hashCart = new HashMap<>();
        String xSql = "select * from Cart where accountId = ?";
        try {
    ...
    public void updateCart(int accountId, int productId, int quantity) {
        String xSql = "update Cart set quantity=? where accountId=? and productId=?";
        try {
            PreparedStatement ps = connection.prepareStatement(xSql);
    ...
    public void addToCart(int accountId, int productId, int quantity) {
        String xSql = "insert into Cart values (?, ?, ?)";
        try {
            PreparedStatement ps = connection.prepareStatement(xSql);
            ps.setInt(1, accountId);
...
//        ProductDAO.INSTANCE.getScreenSizeList().forEach((e) -> {System.out.println(e);});
        ...
        HashMap<Integer, Integer> cart = p.getCart(2);
        
        for (HashMap.Entry<Integer, Integer> entry : cart.entrySet()) {
            System.out.println(entry.getKey() + "\t" + entry.getValue());
        }
//        List<ProductWithImage> list = new ArrayList<>();
//        list = p.getProductListWithImage();
//        for (ProductWithImage productWithImage : list) {
//            System.out.println(productWithImage.getQuantity());
//        }
//        ProductWithImage p1 = p.getProductWithImageByPid(2);
//        System.out.println(p1.getImageText());
    }
}
